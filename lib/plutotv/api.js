(function() {
	...
	const VARIANT_CONCURRENCY = 25;
	const VARIANT_TIMEOUT_MS = 8000;

	const resolveHighestVariant = async (masterUrl, headers) => {
		try {
			...
			if (typeof text !== 'string' || !text.includes('#EXTM3U')) return masterUrl;

			const lines = text.split(/\r?\n/);
			let best = { bandwidth: -1, url: null };
				...
				if (m) {
					...
					if (next && !next.startsWith('#')) {
						if (bw > best.bandwidth) {
							best = { bandwidth: bw, url: new URL(next, masterUrl).toString() };
						}
					}
				}
			}
			...
			console.log(`WARN: failed to resolve highest variant for ${masterUrl}: ${err.message}`);
			return masterUrl;
		}
	};

	const resolveVariantsParallel = async (tasks, headers, concurrency) => {
		...
		const workers = Array.from({ length: Math.min(concurrency, tasks.length) }, async () => {
			while (true) {
				...
				const variant = await resolveHighestVariant(tasks[i], headers);
				results[i] = variant;
				if (variant === tasks[i]) fellBack++; else resolved++;
			}
		});
		await Promise.all(workers);
		console.log(`variant resolution: ${resolved} pinned, ${fellBack} fell back to master (of ${tasks.length})`);
		return results;
	};
	...
	const generateM3U8 = async (
		...
		for (let i = 0; i < channelList.data.length; i++) {
			...
			eligible.push({ c, category });
			masterUrls.push(`${bootData.servers.stitcher}/v2${c.stitched.path}?${bootData.stitcherParams}&jwt=${bootData.sessionToken}&masterJWTPassthrough=true`);
		}
		...
		if (xff) variantHeaders['X-Forwarded-For'] = xff;
		console.log(`resolving highest variants for ${masterUrls.length} channels (region=${region}, concurrency=${VARIANT_CONCURRENCY})`);
		const resolvedUrls = await resolveVariantsParallel(masterUrls, variantHeaders, VARIANT_CONCURRENCY);
		console.log(`done resolving variants for region=${region}`);

		for (let i = 0; i < eligible.length; i++) {
			const { c, category } = eligible[i];
